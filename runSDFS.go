	...
	for {
		...
		case "15":
			...
			if len(params) != 5{
				...
				printAndLogTime("Starting timing!")
				start := time.Now()
				_ = client.startMaple("Maple", params[1], "", mapleNum, params[3], params[4], "", "", INTRO_IP)
				end := time.Now()
				printAndLogTime("Maple took " + strconv.FormatFloat(end.Sub(start).Seconds(), 'f', 3, 64) +"sec.")
			}
		case "16":
			fmt.Println("Please input: juice juiceExe mapleNum interPrefix resultFileName deleteIntermediateFile")
			result, err := reader.ReadString('\n')
			errHandler(err, false, "Read input failed")
			input := strings.TrimSuffix(result, "\n")
			params := strings.Split(input, " ")
			if len(params) != 6{
				...
				printAndLogTime("Starting timing!")
				start := time.Now()
				_ = client.startMaple("Juice", "", params[1], mapleNum, params[3], "", params[4], params[5], INTRO_IP)
				end := time.Now()
				printAndLogTime("Juice took " + strconv.FormatFloat(end.Sub(start).Seconds(), 'f', 3, 64) +"sec.")
			}
		case "17":
			fmt.Println("Please input: maplejuice mapleExe juiceExe mapleNum interPrefix dataDir2 resultFileName deleteIntermediateFile")
			result, err := reader.ReadString('\n')
			errHandler(err, false, "Read input failed")
			input := strings.TrimSuffix(result, "\n")
			params := strings.Split(input, " ")
			if len(params) != 8{
				...
				mapleNum,_ := strconv.Atoi(params[3])
				//mapleJuice(params[1], params[2], mapleNum, params[4], params[5], params[6], params[7])
				printAndLogTime("Starting timing!")
				start := time.Now()
				_ = client.startMaple("MapleJuice", params[1], params[2], mapleNum, params[4], params[5], params[6], params[7], INTRO_IP)
				end := time.Now()
				printAndLogTime("MapleJuice took " + strconv.FormatFloat(end.Sub(start).Seconds(), 'f', 3, 64) +"sec.")
			}
